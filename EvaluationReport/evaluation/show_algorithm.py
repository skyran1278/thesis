...
INPUT_FILE = '../data/20190327 173536 SmartCut.xlsx'
...
def plot_bar_length(top_rebar, top_length, bot_rebar, bot_length, color):
    plt.plot([START, (START + top_length[0]), (START + top_length[0]), (START + top_length[0] + top_length[1]),
              END - top_length[2], END], np.repeat(top_rebar, 2), color=color, linewidth=linewidth)
...
for INDEX in (1252, 1672, 1256, 332, 428, 860, 932, 628, 780, 708, 1164, 24, 288, 1172):
    INDEX = INDEX // 4 * 4
    print(INDEX)
    ...
    if not os.path.exists(f'{SCRIPT_DIR}/{INPUT_FILE}.pkl'):
        print("Reading excel...")
        DATASET = pd.read_excel(SCRIPT_DIR + f'/{INPUT_FILE}',
                                sheet_name='etabs_design')
        BEAM_3 = pd.read_excel(SCRIPT_DIR + f'/{INPUT_FILE}',
                               sheet_name='多點斷筋', header=[0, 1], usecols=22)
        BEAM_3 = BEAM_3.rename(
            columns=lambda x: x if 'Unnamed' not in str(x) else '')
        BEAM_CON = pd.read_excel(SCRIPT_DIR + f'/{INPUT_FILE}',
